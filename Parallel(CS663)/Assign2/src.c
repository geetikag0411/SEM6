...
int main(int argc, char *argv[])
{
    ...
    for (int n = 1; n <= num_time_steps; n++)
    {
        ...
        { // i.e. myrank%Px != Px - 1 i.e. this is not the last process in the "row"
            ...
            for (int i = 0; i < side; i++)
            {
                MPI_Pack(&data_old[i][side - 1], 1, MPI_DOUBLE, sendbuffer, 2 * side * sizeof(double), &position, MPI_COMM_WORLD);
            }
            for (int i = 0; i < side; i++) // pack the second last column
                MPI_Pack(&data_old[i][side - 2], 1, MPI_DOUBLE, sendbuffer, 2 * side * sizeof(double), &position, MPI_COMM_WORLD);
        ...
        if (myrank % Px > 0)
        {
            ...
            {
                MPI_Pack(&data_old[i][0], 1, MPI_DOUBLE, sendbuffer, 2 * side * sizeof(double), &position, MPI_COMM_WORLD);
            }
            for (int i = 0; i < side; i++) // pack the second column
            {
                MPI_Pack(&data_old[i][1], 1, MPI_DOUBLE, sendbuffer, 2 * side * sizeof(double), &position, MPI_COMM_WORLD);
            }
        ...
        if ((myrank / Px) < Py - 1)
        {
            ...
            int position = 0;
            for(int j = 0; j < side; j++)
                MPI_Pack(&data_old[side - 2][j],1,MPI_DOUBLE, sendbuffer, 2 * side * sizeof(double), &position, MPI_COMM_WORLD );
            for(int j = 0; j < side; j++)
                MPI_Pack(&data_old[side - 1][j],1,MPI_DOUBLE, sendbuffer, 2 * side * sizeof(double), &position, MPI_COMM_WORLD );
        ...
        if ((myrank / Px) > 0)
        {
            ...
            // this packing is necessary since on malloc, rows of data may not be contiguous in memory 
            int position = 0;
            for(int j = 0; j < side; j++)
                MPI_Pack(&data_old[0][j],1,MPI_DOUBLE, sendbuffer, 2 * side * sizeof(double), &position, MPI_COMM_WORLD );
            for(int j = 0; j < side; j++)
                MPI_Pack(&data_old[1][j],1,MPI_DOUBLE, sendbuffer, 2 * side * sizeof(double), &position, MPI_COMM_WORLD );
            //do gather at the leader
            MPI_Gather(sendbuffer, side * 2, MPI_DOUBLE, leader_send_buffer, side * 2, MPI_DOUBLE, 0, newcomm);
